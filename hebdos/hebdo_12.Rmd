...
for (i in 1:length(angles_cols)) {
  ...
  formule <- as.formula(paste("perte_ndvi ~", var_angle))
  ...
  results$estimate[i] <- summary(model)$coefficients[var_angle, "Estimate"]
}
...
bestestimate <- which.max(abs(results$estimate))
...
plot(seq(2,20,2), results$estimate, type = "b", pch = 19, col = "orange",
     xlab = "Angle d'inflexion (°)", ylab = "Estimate",
     main = "Comparaison de l'estimate")
points(seq(2,20,2)[bestestimate], results$estimate[bestestimate], col = "red", cex = 2, lwd = 2)
